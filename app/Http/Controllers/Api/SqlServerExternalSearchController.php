...
class SqlServerExternalSearchController extends Controller
{
    public function __invoke(Request $request, SqlServerSearchService $searchService): JsonResponse
    {
        ...
        try {
            ...
            $originCountry = $this->resolveOriginCountry($result['packageRows'], $result['codigo']);
    ...
    private function transformExternalEvents($trackingRows, string $originCountry)
    {
        return collect($trackingRows)
            ->map(function ($row) use ($originCountry) {
                ...
                return [
                    ...
                    'office' => $this->buildOffice($row, $originCountry),
    ...
    private function buildOffice(object $row, string $originCountry): string
    {
    ...
    private function resolveOriginCountry($packageRows, string $codigo): string
    {
        $package = collect($packageRows)->first();

        $countryCode = isset($package->ORIG_COUNTRY_CD) ? trim((string) $package->ORIG_COUNTRY_CD) : '';
        $countryName = isset($package->ORIG_COUNTRY_NM) ? trim((string) $package->ORIG_COUNTRY_NM) : '';
